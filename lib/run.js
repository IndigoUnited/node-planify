...
function hookStepStdio(step, notify) {
    let stdioPromise = Promise.resolve();

    if (step.options.mute) {
        stdio.stdout.hook(() => {});
        stdio.stderr.hook(() => {});
    } else {
        stdioPromise = Promise.resolve();
        stdio.stdout.hook((buffer, encoding, write) => {
            stdioPromise = stdioPromise.then(() =>
                notify(step, 'write.stdout', buffer, encoding, write));
        });
        stdio.stderr.hook((buffer, encoding, write) => {
            stdioPromise = stdioPromise.then(() =>
                notify(step, 'write.stderr', buffer, encoding, write));
        });
    }
